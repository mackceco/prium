...
#if defined(NDEBUG)
#error "prium cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "prium Signed Message:\n";
...
// prium: total coin age spent in transaction, in the unit of coin-days.
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		 ...
		 * note we only undo zerocoin databasing in the following statement, value to and from prium
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
	...
	if (miPrev != mapBlockIndex.end()) {
		...
		// prium: compute chain trust score
		pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

		// prium: compute stake entropy bit for stake modifier
		if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
			LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

		// prium: record proof-of-stake hash value
		if (pindexNew->IsProofOfStake()) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// prium: We use certain sporks during IBD, so check to see if they are available. If not, ask the first peer connected for them.
