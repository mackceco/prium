...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a prium address (e.g. %1)").arg("NqpjKV7LyS6SiZGh9ve3CReezksCqqpz7b"));
#endif
...
{
    // return if URI is not valid or is no prium: URI
...
{
    // Convert prium:// to prium:
    //
    //    Cannot handle this later, because prium:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open prium.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "prium.lnk";
}
...
{
    return GetAutostartDir() / "prium.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a prium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=prium\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the prium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add prium app to startup item list
