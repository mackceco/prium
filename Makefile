...
pkgdatadir = $(datadir)/prium
pkgincludedir = $(includedir)/prium
pkglibdir = $(libdir)/prium
pkglibexecdir = $(libexecdir)/prium
...
CONFIG_HEADER = $(top_builddir)/src/config/prium-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/prium-config.h.in \
...
BITCOIN_CLI_NAME = prium-cli
BITCOIN_DAEMON_NAME = priumd
BITCOIN_GUI_NAME = prium-qt
BITCOIN_TX_NAME = prium-tx
...
OTOOL64 = 
PACKAGE = prium
PACKAGE_BUGREPORT = www.prium.org
PACKAGE_NAME = prium Core
PACKAGE_STRING = prium Core 4.0.0
PACKAGE_TARNAME = prium
PACKAGE_URL = 
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_prium_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_prium_coverage.info test_prium.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/prium-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/prium-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/prium-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/prium-config.h
$(top_srcdir)/src/config/prium-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/prium-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/prium-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/prium-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/prium-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_prium.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_prium -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_prium_filtered.info: test_prium.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_prium_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_prium_coverage.info: baseline_filtered_combined.info test_prium_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_prium_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_prium_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_prium_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_prium.coverage/.dirstamp:  test_prium_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_prium.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_prium.coverage/ total.coverage/ $(OSX_APP)
