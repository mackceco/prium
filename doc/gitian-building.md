...
*Setup instructions for a gitian build of prium using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the prium
...
Clone the git repositories for prium and gitian and then checkout the prium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the prium repository
...
To build prium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the prium repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/prium/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/prium-core/prium.git
...
./bin/gbuild --commit prium=${COMMIT} --url prium=${URL} ../prium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit prium=${COMMIT} --url prium=${URL} ../prium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit prium=${COMMIT} --url prium=${URL} ../prium/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/prium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/prium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/prium-build.assert
```
...
[prium-core/gitian.sigs](https://github.com/prium-core/gitian.sigs/) repository, or if that's not possible to create a pull
request.
